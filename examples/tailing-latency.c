...
void* read_proc(void* unused) {
	...
	while (1) {
		...
		while (1) {
			ssize_t n = nfs_read(nfs, fr, &rpage, rwsize);
            t2 = rpage.t;
...
int main(int argc, char *argv[]) {
  ...
  #define SetFlags(f) flags = f, str_flags = #f;
    if (atoi(getenv("O_DSYNC")?:"1")) {
        SetFlags(O_CREAT|O_WRONLY|O_DSYNC);
    } else {
        SetFlags(O_CREAT|O_WRONLY);
    }
    dsync = atoi(getenv("dsync")?:"0");
	fw = open(argv[1], flags, 0777);
	if (fw < 0) {
		fprintf(stderr, "open(%s, %s, 0777) = %m\n", argv[1], str_flags);
		return 1;
	}
